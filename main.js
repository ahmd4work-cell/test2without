document.addEventListener('DOMContentLoaded', function() {
    ...
    // 1. قراءة البيانات وربطها بنظام الطلبات كما في صفحة المبيعات تماماً
    function getRawData() {
        try {
            ...
            // حساب الإجماليات لكل طلب (مطابق تماماً لصفحة المبيعات)
    ...
    function updateDashboard() {
        ...
        // تجميع القيم المباشرة من الطلبات المفلترة
    ...
    function initCharts() {
        ...
        // إضافة دالة مساعدة لرسم الإبرة (المؤشر)
        const gaugeNeedlePlugin = {
            ...
            afterDatasetDraw(chart, args, options) {
                ...
                // حساب الزاوية (نصف دائرة تبدأ من اليسار لليمين)
                const angle = Math.PI + (Math.PI * (percent / 100));
                
                // تحديد المنتصف من خصائص الرسم
                ...
                ctx.lineTo(0, 3.5);
                ctx.fillStyle = '#0a3a22'; // أخضر داكن (لون المؤسسة)
                ctx.fill();
                
                // رسم النقطة في المنتصف
                ctx.beginPath();
        ...
        if (gaugeEl) {
            gaugeChart = new Chart(gaugeEl.getContext('2d'), {
                ...
                data: { 
                    datasets: [{ 
                        data: [100], // قطعة واحدة تشمل كامل نصف الدائرة
                        backgroundColor: function(context) {
                            const chart = context.chart;
                            const {ctx, chartArea} = chart;
                            if (!chartArea) return null;
                            // عمل تدرج لوني 3 ألوان متداخلة
                            const gradient = ctx.createLinearGradient(chartArea.left, 0, chartArea.right, 0);
                            gradient.addColorStop(0, '#ef4444'); // أحمر (البداية)
                            gradient.addColorStop(0.5, '#fbbf24'); // أصفر (المنتصف)
                            gradient.addColorStop(1, '#22c55e'); // أخضر (النهاية)
                            return gradient;
                        },
                        borderWidth: 0 
                    }] 
                ...
                options: { 
                    rotation: 270, 
                    circumference: 180, 
                    cutout: '80%', // تفريغ الدائرة من الداخل
                    responsive: true, 
                    maintainAspectRatio: false, 
                    plugins: { 
                        legend: { display: false },
                        tooltip: { enabled: false }, // إخفاء الملاحظة عند التمرير 
                        gaugeNeedle: { percent: 0 } // النسبة المبدئية
                    } 
                }
            });
        }
    ...
    function updateChartsLogic(salesTotal, pendingTotal, totalVisits, successVisits, filteredSales = []) {
        ...
        // حساب نسبة مؤشر الإنجاز (التارجت السنوي)
        ...
        // تحديث إبرة المؤشر الديناميكية
        if (gaugeChart) { 
            const safePercent = Math.min(gaugePercentRaw, 100); // لا يتجاوز 100% في المؤشر
