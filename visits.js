...
/* ==========================================================
   6. العمليات التشغيلية وتحديث التواريخ ونقل الفرص
   ========================================================== */
function updateEditDateField(row) {
    ...
    const mainContainer = row.querySelector('.edit-date-container-main');
    if (mainContainer) mainContainer.innerHTML = parseEditDateHTML(fullDateTime);
}
...
async function handleStatusChange(selectEl, rowId) {
    const newVal = selectEl.value; const oldVal = selectEl.dataset.old; const mainRow = selectEl.closest('.main-row'); const companyName = mainRow.cells[1].querySelector('input').value;
    applyStatusColor(selectEl); addToActivityLog('الحالة', oldVal, newVal, companyName); updateEditDateField(mainRow); saveAllDataSilently(); updateStats(); selectEl.dataset.old = newVal;

    if (newVal === 'تأهيل لفرصة') {
        const result = await Swal.fire({ title: 'تأهيل لفرصة', text: "هل تريد نقل هذه الزيارة إلى جدول الفرص البيعية؟", icon: 'question', showCancelButton: true, confirmButtonColor: '#3b82f6', cancelButtonColor: '#94a3b8', confirmButtonText: 'نعم، انقله', cancelButtonText: 'إلغاء' });
        if (result.isConfirmed) {
            transferRowToOpportunities(mainRow, rowId);
        }
    }
}

function transferRowToOpportunities(mainRow, rowId) {
    const subRow = document.getElementById('sub-' + rowId);
    const products = [];
    if (subRow) {
        subRow.querySelectorAll('.product-body tr').forEach(pRow => {
    ...
    let opps = JSON.parse(localStorage.getItem(OPPORTUNITIES_KEY) || '[]');
    opps.unshift(oppData);
    localStorage.setItem(OPPORTUNITIES_KEY, JSON.stringify(opps));
    Swal.fire({icon: 'success', title: 'تم النقل', text: 'تم نقل الزيارة بنجاح إلى جدول الفرص البيعية', showConfirmButton: false, timer: 1500});
}
...
function applyStatusColor(selectEl) {
    ...
    if (val === 'تأهيل لفرصة' || val === 'عرض سعر') {
        selectEl.classList.add('status-yellow');
    } else if (val === 'فقدان' || val === 'غير مهتم') {
...
function reorderRows() {
    ...
    const rowsData = rows.map(row => {
        ...
        return { row: row, subRow: document.getElementById('sub-' + row.id), date: dateInput ? dateInput.value : today };
    });
    ...
    sortedMonths.forEach(month => {
        ...
        const monthText = isCurrentMonth ? `زيارات شهر ${month} (الحالي)` : `زيارات شهر ${month}`;
...
function loadSavedData() { const rawData = localStorage.getItem(STORAGE_KEY); const tbody = document.getElementById('tableBody'); if (!tbody) return; tbody.innerHTML = ''; if (rawData) { JSON.parse(rawData).forEach(v => renderRow(v, false)); } else { renderRow({}, false); } reorderRows(); updateStats(); renderActivityLog(); }
