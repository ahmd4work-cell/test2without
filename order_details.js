...
function toggleLogExpansion() {
    ...
    const footerCells = document.querySelectorAll('.table-footer td');
    if (section.classList.contains('expanded')) {
        section.classList.remove('expanded');
        btn.innerHTML = '<i class="fas fa-expand-alt"></i>';
        // إرجاع الفوتر ليلامس السجل المُصغر
        footerCells.forEach(c => c.style.bottom = '0px');
    } else {
        section.classList.add('expanded');
        btn.innerHTML = '<i class="fas fa-compress-alt"></i>';
        // رفع الفوتر للأعلى ليتماشى مع السجل بعد توسعته
        footerCells.forEach(c => c.style.bottom = '80px');
    }
}
...
// تعديل صيغة وتصميم نص النشاط ليكون مطابقاً للزيارات (المستخدم | الثلاثاء 2026-07-28 08:35 | ...)
...
function renderProducts(filtered = null) {
    ...
    items.forEach((p) => {
        ...
        const pIden = p.name || 'بدون تفاصيل'; // تم تعديله ليكون اسم المنتج بناءً على طلبك
