...
function renderProducts(filtered = null) {
    ...
    // الإضافة لضمان امتداد مساحة الجدول لدفع الشريط الأسود للأسفل وتثبيته فوق سجل النشاط
    tbody.innerHTML += `<tr class="filler-row" style="height: 100%;"><td colspan="12" style="border: none; background: transparent; pointer-events: none; padding: 0;"></td></tr>`;
...
function saveGlobalNote() {
    ...
    if (newText) {
        ...
        // التعديل: وضع العنوان في حاوية مرنة منفصلة ليكون في السطر الأول، والنص في حاوية أسفله تأخذ العرض بالكامل
...
function handleFileUpload(event) {
    ...
    // التعديل: فصل العنوان عن نص إرفاق الملف
    let newEntry = `<div style="width: 100%; display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                        ${generateStyledHeaderForNotes()}
                    </div>
