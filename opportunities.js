...
function updateAllDateColors() {
    ...
    document.querySelectorAll('#tableBody .main-row').forEach(row => {
        ...
        if (diffDays < 0) {
            dateInput.classList.add('date-past');       
        } else if (diffDays === 0) {
...
/* ==========================================================
   9. منطق الـ Date Picker المنبثق
   ========================================================== */
...
let dpActiveRowId = null;

let dpTopInput, dpMonthLabel, dpYearLabel, dpDatesGrid, dpDropdown;

function initDatePicker() {
    dpTopInput = document.getElementById("dpTopInput");
    dpMonthLabel = document.getElementById("dpMonthLabel");
    dpYearLabel = document.getElementById("dpYearLabel");
    dpDatesGrid = document.getElementById("dpDatesGrid");
    dpDropdown = document.getElementById("dpDropdown");

    const cancelBtn = document.getElementById("dpCancelBtn");
    const nextBtn = document.getElementById("dpNextBtn");
    const prevMonth = document.getElementById("dpPrevMonth");
    const nextMonth = document.getElementById("dpNextMonth");
    const prevYear = document.getElementById("dpPrevYear");
    const nextYear = document.getElementById("dpNextYear");

    cancelBtn.addEventListener("click", closeCustomDatePicker);
    
    nextBtn.addEventListener("click", () => {
        if (dpSelectedDate) {
            const yyyy = dpCurrentYear;
            const mm = String(dpCurrentMonth + 1).padStart(2, '0');
            const dd = String(dpSelectedDate).padStart(2, '0');
            applySelectedDate(`${yyyy}-${mm}-${dd}`);
        } else {
    ...
    prevMonth.addEventListener("click", () => {
        ...
        dpSelectedDate = null; renderCustomCalendar();
    });
    ...
    prevYear.addEventListener("click", () => { dpCurrentYear--; dpSelectedDate = null; renderCustomCalendar(); });
    nextYear.addEventListener("click", () => { dpCurrentYear++; dpSelectedDate = null; renderCustomCalendar(); });

    dpMonthLabel.addEventListener("click", () => {
        dpDropdown.innerHTML = "";
        dpMonthNames.forEach((name, index) => {
            const item = document.createElement("div");
            item.className = "dp-dropdown-item";
            item.textContent = name;
            item.addEventListener("click", () => {
                dpCurrentMonth = index; dpSelectedDate = null; dpDropdown.style.display = "none"; renderCustomCalendar();
            });
    ...
    dpYearLabel.addEventListener("click", () => {
        ...
        for (let y = 2000; y <= 2050; y++) {
            ...
            item.textContent = y;
            item.addEventListener("click", () => {
                dpCurrentYear = y; dpSelectedDate = null; dpDropdown.style.display = "none"; renderCustomCalendar();
            });
            dpDropdown.appendChild(item);
        }
        dpDropdown.style.display = "block";
...
function renderCustomCalendar() {
    ...
    // إعداد تاريخ اليوم الحالي للمقارنة
    const todayDateObj = new Date();
    const currentTodayTime = new Date(todayDateObj.getFullYear(), todayDateObj.getMonth(), todayDateObj.getDate()).getTime();

    for (let i = 0; i < firstDay; i++) {
        const emptyCell = document.createElement("div");
        emptyCell.className = "dp-date dp-empty";
        dpDatesGrid.appendChild(emptyCell);
    }

    for (let d = 1; d <= daysInMonth; d++) {
        const cell = document.createElement("div");
        cell.className = "dp-date";
        cell.textContent = d;

        // مقارنة التواريخ لتعيين الألوان المناسبة
        const loopDateTime = new Date(dpCurrentYear, dpCurrentMonth, d).getTime();
        
        if (loopDateTime < currentTodayTime) {
            cell.classList.add("dp-past"); // تاريخ ماضي (خافت)
        } else if (loopDateTime === currentTodayTime) {
            cell.classList.add("dp-today"); // تاريخ اليوم (دائرة سوداء)
        }

        if (d === dpSelectedDate) { cell.classList.add("dp-selected"); }

        cell.addEventListener("click", (e) => {
            e.stopPropagation(); // منع الإغلاق التلقائي عند اختيار يوم
            dpSelectedDate = d;
...
function openCustomDatePicker(e, inputEl, rowId) {
    if(e) e.stopPropagation(); // منع انتقال حدث الضغط للخلفية
    ...
    if (inputEl.value) {
        ...
        const d = new Date();
        dpCurrentYear = d.getFullYear();
        dpCurrentMonth = d.getMonth();
        dpSelectedDate = null;
    }

    renderCustomCalendar();
    
    // إظهار وحساب إحداثيات ظهور القائمة بناءً على مكان الحقل
    ...
    // تعديل الموضع إذا خرج التقويم عن الشاشة
    setTimeout(() => {
        const popupRect = popup.getBoundingClientRect();
        if (leftPos + popupRect.width > window.innerWidth) {
            leftPos = window.innerWidth - popupRect.width - 10;
        }
        if (topPos + popupRect.height > window.innerHeight + window.scrollY) {
            topPos = rect.top + window.scrollY - popupRect.height - 4; 
        }
...
function applySelectedDate(dateStr) {
    ...
    const oldVal = dpActiveInput.dataset.old || '';
    dpActiveInput.value = dateStr;

    const mainRow = document.getElementById(dpActiveRowId);
    if (mainRow) {
        const companyName = mainRow.cells[1].querySelector('input').value;
        addToActivityLog('التاريخ المتوقع', oldVal, dateStr, companyName);
        dpActiveInput.dataset.old = dateStr;
...
// دالة إغلاق التقويم وقوائمه عند الضغط في أي مكان فارغ بالصفحة
document.addEventListener('click', function(e) {
    const popup = document.getElementById('datePickerPopup');
    if (popup && popup.style.display === 'block') {
        if (!popup.contains(e.target) && !e.target.classList.contains('exp-date-input')) {
            closeCustomDatePicker();
        }
    }
    // إغلاق القائمة المنسدلة للشهور/السنوات إذا ضغطت خارجها
