...
function renderRow(v = {}, prepend = false) {
    ...
    mainRow.innerHTML = `
        ...
        <td>
            <input type="text" class="excel-input exp-date-input" placeholder="اختر التاريخ" value="${v.expDate || ''}" data-old="${v.expDate || ''}" onfocus="this.dataset.old=this.value" onchange="addToActivityLog('التاريخ المتوقع', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value; debouncedSaveAllData(); reorderRows();">
    ...
    // تفعيل التقويم (Flatpickr) على حقل التاريخ المتوقع بعد إضافته للصفحة
    flatpickr(mainRow.querySelector('.exp-date-input'), {
        dateFormat: "Y-m-d",
        disableMobile: "true" // لإجبار الهاتف على استخدام هذا التقويم المخصص
    });
}
