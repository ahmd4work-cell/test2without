...
const STORAGE_KEY = 'asgate_opportunities_final_v32';
...
function renderRow(v = {}, prepend = false) {
    ...
    mainRow.innerHTML = `
        ...
        <td>
            <input type="text" class="excel-input exp-date-input" readonly style="cursor:pointer;" placeholder="اختر التاريخ" value="${v.expDate || ''}" data-old="${v.expDate || ''}" onclick="openCustomDatePicker(this)">
...
function calculateMainVisitValue(rowId) {
    ...
    if (mainRow) { const oppVal = mainRow.querySelector('.opp-value-input'); if (oppVal) oppVal.value = grandTotal > 0 ? grandTotal : ''; }
    debouncedSaveAllData();
}

/* ==========================================================
   4. الإجراءات الجماعية والبحث والفرز وإدارة الملاحظات
   ========================================================== */
...
async function handleBulkAction(action) {
    ...
    } else if (action === 'تصدير') { exportToExcel(selected); addToActivityLog('إجراء', 'تصدير بيانات الفرص للإكسيل', '', 'مجموعة محددة'); } 
      else if (action === 'طباعة') { printSelected(selected); addToActivityLog('إجراء', 'طباعة بيانات الفرص', '', 'مجموعة محددة'); } 
      else { selected.forEach(chk => { const row = chk.closest('tr'); addToActivityLog('إجراء', action, '', row.cells[1].querySelector('input').value); }); Swal.fire({icon: 'success', title: 'تم', text: 'تم تنفيذ الإجراء', showConfirmButton: false, timer: 1500}); }
}

function exportToExcel(selectedRows) { let csvContent = "\uFEFFالشركة,العنوان,المسؤول,رقم التواصل,الإيميل,السجل,تاريخ الفرصة,الخدمة,القيمة,الحالة,التاريخ المتوقع,المالك\n"; selectedRows.forEach(chk => { const row = chk.closest('tr'); const getVal = (idx) => { const inp = row.cells[idx].querySelector('input, select'); return `"${inp ? inp.value.replace(/"/g, '""') : ''}"`; }; csvContent += [getVal(1), getVal(2), getVal(3), getVal(4), getVal(5), getVal(6), getVal(7), getVal(8), getVal(9), getVal(11), getVal(12), getVal(13)].join(",") + "\n"; }); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = "تقرير_الفرص_البيعية_" + getTodayFormatted() + ".csv"; document.body.appendChild(link); link.click(); document.body.removeChild(link); }
function printSelected(selectedRows) { let printWindow = window.open('', '_blank'); let html = `<html dir="rtl"><head><title>تقرير الفرص البيعية المحددة</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet"><style>body { font-family: 'Cairo', sans-serif; padding: 20px; color: #0f172a; } h2 { text-align: center; color: #4c1d95; margin-bottom: 20px; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; } table { width: 100%; border-collapse: collapse; font-size: 12px; } th { background-color: #f1f5f9; color: #1e293b; padding: 10px; border: 1px solid #cbd5e1; font-weight: 700; } td { padding: 8px; border: 1px solid #cbd5e1; text-align: center; } tr:nth-child(even) { background-color: #f8fafc; } .footer { margin-top: 30px; text-align: left; font-size: 10px; color: #64748b; }</style></head><body><h2>تقرير الفرص البيعية</h2><table><thead><tr><th>الشركة</th><th>المسؤول</th><th>رقم التواصل</th><th>الخدمة</th><th>تاريخ الفرصة</th><th>الحالة</th></tr></thead><tbody>`; selectedRows.forEach(chk => { const row = chk.closest('tr'); const getVal = (idx) => row.cells[idx].querySelector('input, select')?.value || ''; html += `<tr><td><strong>${getVal(1)}</strong></td><td>${getVal(3)}</td><td dir="ltr">${getVal(4)}</td><td>${getVal(8)}</td><td>${getVal(7)}</td><td>${getVal(11)}</td></tr>`; }); html += `</tbody></table><div class="footer">تاريخ الطباعة: ${getTodayFormatted()}</div></body></html>`; printWindow.document.write(html); printWindow.document.close(); setTimeout(() => { printWindow.print(); printWindow.close(); }, 500); }
function debouncedFilterTable() { clearTimeout(searchTimeout); searchTimeout = setTimeout(filterTable, 300); }
function filterTable() { const q = document.getElementById('searchInput').value.toLowerCase().trim(); document.querySelectorAll('.main-row').forEach(row => { const text = Array.from(row.cells).slice(1, 7).map(c => c.querySelector('input')?.value.toLowerCase() || '').join(' '); const subRow = document.getElementById('sub-' + row.id); if (text.includes(q)) { row.style.display = 'table-row'; } else { row.style.display = 'none'; if(subRow) subRow.style.display = 'none'; } }); }

function openNote(el) {
    currentActivePreview = el; let arr = []; try { arr = JSON.parse(el.getAttribute('data-full-notes') || "[]"); } catch(e) {}
    const historyLog = document.getElementById('historyLog'); const days = ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'];
    if (historyLog) { historyLog.innerHTML = arr.map(msg => { let msgDateObj = new Date(msg.date); let dayStr = isNaN(msgDateObj) ? '' : days[msgDateObj.getDay()] + ' '; let userName = msg.user && msg.user !== "المستخدم" ? msg.user : "المستخدم"; return `<div class="log-entry" style="display: block; line-height: 1.6;"><div style="margin-bottom: 4px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;"><span class="log-badge-user"><i class="fas fa-user-circle"></i> ${userName}</span><span class="log-divider">|</span><span class="log-timestamp"><i class="fas fa-clock"></i> ${dayStr}${msg.date} ${msg.time}</span><span class="log-divider">|</span></div><div class="log-action" style="padding-right: 5px; color: #0f172a; font-size: 11px; font-weight: 700; white-space: pre-wrap; display: block;">${msg.text}</div></div>`; }).join('') || '<div style="color:#64748b; text-align:center; font-size:10px; padding:20px; font-weight:700;">لا توجد ملاحظات سابقة</div>'; }
...
function saveNote() { const txt = document.getElementById('modalTextArea').value.trim(); if (txt && currentActivePreview) { let arr = []; try { arr = JSON.parse(currentActivePreview.getAttribute('data-full-notes') || "[]"); } catch(e) {} let username = "المستخدم"; const mainRow = currentActivePreview.closest('.main-row'); if (mainRow) { const ownerInput = mainRow.cells[13]?.querySelector('input'); if (ownerInput && ownerInput.value.trim()) username = ownerInput.value.trim(); } arr.push({ user: username, date: getTodayFormatted(), time: getTimeFormatted(), text: txt }); currentActivePreview.setAttribute('data-full-notes', JSON.stringify(arr)); currentActivePreview.innerText = txt; if (mainRow) updateEditDateField(mainRow); saveAllDataSilently(); } closeNote(); }
...
/* ==========================================================
   5. تحديث التواريخ والحالة والألوان 
   ========================================================== */
function updateEditDateField(row) { if (!row) return; const dateFormatted = getTodayFormatted(); const time24 = getTimeFormatted(); const fullDateTime = `${dateFormatted} ${time24}`; const hiddenInput = row.querySelector('.edit-date-val'); if (hiddenInput) hiddenInput.value = fullDateTime; const subRow = document.getElementById('sub-' + row.id); if (subRow) { const subContainer = subRow.querySelector('.edit-date-container-sub'); if (subContainer) subContainer.innerHTML = `<span class="edit-date-d">${dateFormatted}</span><span class="edit-date-t">${time24}</span>`; } }
...
async function handleStatusChange(selectEl, rowId) { const newVal = selectEl.value; const oldVal = selectEl.dataset.old; const companyName = selectEl.closest('tr').cells[1].querySelector('input').value; applyStatusColor(selectEl); addToActivityLog('الحالة', oldVal, newVal, companyName); updateEditDateField(selectEl.closest('tr')); saveAllDataSilently(); updateStats(); selectEl.dataset.old = newVal; }

function updateAllDateColors() {
    const todayStr = getTodayFormatted(); const todayObj = new Date(todayStr);
    document.querySelectorAll('#tableBody .main-row').forEach(row => {
        const dateInput = row.querySelector('.exp-date-input'); if(!dateInput) return;
        const status = row.querySelector('.status-select').value;
        dateInput.classList.remove('date-today', 'date-warning', 'date-past');
        if (status === 'رابح' || status === 'فقدان') return;
        const dVal = dateInput.value; if (!dVal) return;
        const expDateObj = new Date(dVal); const diffDays = Math.round((expDateObj - todayObj) / (1000 * 60 * 60 * 24));
...
function applyStatusColor(selectEl) { 
    if (!selectEl) return; const val = selectEl.value; const mainRow = selectEl.closest('.main-row'); 
    selectEl.classList.remove('status-yellow', 'status-green', 'status-red'); if (mainRow) mainRow.classList.remove('closed-row'); 
    if (val === 'مهتم') selectEl.classList.add('status-yellow'); else if (val === 'رابح') { selectEl.classList.add('status-green'); if (mainRow) mainRow.classList.add('closed-row'); } else if (val === 'فقدان') { selectEl.classList.add('status-red'); if (mainRow) mainRow.classList.add('closed-row'); } 
    updateAllDateColors();
}

function reorderRows() { 
    const tbody = document.getElementById('tableBody'); if (!tbody) return; 
    const rows = Array.from(tbody.querySelectorAll('.main-row')); const today = getTodayFormatted(), currentMonth = today.substring(0, 7); 
    const rowsData = rows.map(row => { const expInput = row.querySelector('.exp-date-input'); return { row: row, subRow: document.getElementById('sub-' + row.id), date: (expInput && expInput.value) ? expInput.value : '9999-12-31' }; }); 
    rowsData.sort((a, b) => a.date.localeCompare(b.date)); 
    const groups = {}; rowsData.forEach(item => { const month = item.date === '9999-12-31' ? 'بدون تاريخ متوقع' : item.date.substring(0, 7); if (!groups[month]) groups[month] = []; groups[month].push(item); }); 
    tbody.innerHTML = ''; const fragment = document.createDocumentFragment(); 
    const sortedMonths = Object.keys(groups).sort((a, b) => { if (a === currentMonth) return -1; if (b === currentMonth) return 1; if (a === 'بدون تاريخ متوقع') return 1; if (b === 'بدون تاريخ متوقع') return -1; return b.localeCompare(a); });
    sortedMonths.forEach(month => { 
        const sepRow = document.createElement('tr'); sepRow.className = 'month-separator'; const isCurrentMonth = (month === currentMonth); 
        const sepStyle = isCurrentMonth ? 'background-color: #a855f7 !important; color:#fff !important; box-shadow: 0 2px 4px rgba(168,85,247,0.3);' : 'background-color: #3b82f6 !important; color:#fff !important; box-shadow: 0 2px 4px rgba(59,130,246,0.3);'; 
        ...
        fragment.appendChild(sepRow); groups[month].forEach(item => { fragment.appendChild(item.row); if (item.subRow) fragment.appendChild(item.subRow); }); 
    }); 
    tbody.appendChild(fragment); updateAllDateColors(); 
}
...
function saveAllDataSilently() {
    const data = Array.from(document.querySelectorAll('#tableBody .main-row')).map(row => {
        ...
        return { 
            ...
            expDate: row.cells[12].querySelector('.exp-date-input').value, editDate: row.querySelector('.edit-date-val')?.value || '', owner: row.cells[13].querySelector('input').value, products: products 
        };
    });
...
function updateStats() { const rows = document.querySelectorAll('#tableBody .main-row'); const today = getTodayFormatted(), currentMonth = today.substring(0, 7); let total = rows.length, tDay = 0, tMonth = 0, valTotal = 0, valMonth = 0; rows.forEach(row => { const dateInput = row.querySelector('.opp-date-val'); const visitValInput = row.querySelector('.opp-value-input'); const visitVal = visitValInput ? parseFloat(visitValInput.value) || 0 : 0; valTotal += visitVal; if (dateInput) { const date = dateInput.value; if (date === today) tDay++; if (date.startsWith(currentMonth)) { tMonth++; valMonth += visitVal; } } }); if (document.getElementById('stat-total')) document.getElementById('stat-total').innerText = total; if (document.getElementById('stat-today')) document.getElementById('stat-today').innerText = tDay; if (document.getElementById('stat-month')) document.getElementById('stat-month').innerText = tMonth; if (document.getElementById('stat-value-total')) document.getElementById('stat-value-total').innerText = valTotal.toLocaleString() + ' ر.س'; if (document.getElementById('stat-value-month')) document.getElementById('stat-value-month').innerText = valMonth.toLocaleString() + ' ر.س'; }
function addToActivityLog(fieldName, oldVal, newVal, companyName) { if (oldVal === newVal) return; const days = ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت']; const d = new Date(); let dd = String(d.getDate()).padStart(2, '0'), mm = String(d.getMonth() + 1).padStart(2, '0'), yyyy = d.getFullYear(); const dayName = days[d.getDay()]; const timeStr = getTimeFormatted(); const cleanCompany = companyName || 'شركة غير مسماة'; let actionText = fieldName === 'إجراء' ? `${oldVal} لفرصة شركة ( ${cleanCompany} )` : `تعديل ${fieldName} من [${oldVal || 'فارغ'}] إلى [${newVal || 'فارغ'}] لفرصة العميل ( ${cleanCompany} )`; const fullLogHTML = `<div class="log-entry"><span class="log-badge-user"><i class="fas fa-user-circle"></i> المستخدم</span><span class="log-divider">|</span><span class="log-timestamp"><i class="fas fa-clock"></i> ${dayName} ${yyyy}-${mm}-${dd} ${timeStr}</span><span class="log-divider">|</span><span class="log-action">${actionText}</span></div>`; let logs = JSON.parse(localStorage.getItem(LOGS_KEY) || '[]'); logs.unshift(fullLogHTML); localStorage.setItem(LOGS_KEY, JSON.stringify(logs.slice(0, 100))); renderActivityLog(); }
...
/* ==========================================================
   7. منطق وتشغيل التقويم المخصص (Custom Date Picker)
   ========================================================== */
const dpMonthNames = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
let dpCurrentYear = 2026;
let dpCurrentMonth = 6;
let dpSelectedDate = null;
let activeDateInput = null;

const dpTopInput = document.getElementById("dpTopInput");
const dpMonthLabel = document.getElementById("dpMonthLabel");
const dpYearLabel = document.getElementById("dpYearLabel");
const dpDatesGrid = document.getElementById("dpDatesGrid");
const dpDropdown = document.getElementById("dpDropdown");
...
function openCustomDatePicker(inputEl) {
    ...
    const currentVal = inputEl.value; // YYYY-MM-DD format expected
    if (currentVal && currentVal.includes('-')) {
        const parts = currentVal.split('-');
        dpCurrentYear = parseInt(parts[0], 10);
        dpCurrentMonth = parseInt(parts[1], 10) - 1;
        dpSelectedDate = parseInt(parts[2], 10);
    } else {
        const today = new Date();
        dpCurrentYear = today.getFullYear();
        dpCurrentMonth = today.getMonth();
        dpSelectedDate = today.getDate();
    }
...
function updateDpTopInput() {
    if (dpSelectedDate) {
        dpTopInput.textContent = `${dpMonthNames[dpCurrentMonth]} ${dpSelectedDate}, ${dpCurrentYear}`;
    } else {
        dpTopInput.textContent = `${dpMonthNames[dpCurrentMonth]} ${dpCurrentYear}`;
    }
}

function renderCustomCalendar() {
    dpMonthLabel.textContent = dpMonthNames[dpCurrentMonth];
    dpYearLabel.textContent = dpCurrentYear;
    dpDatesGrid.innerHTML = "";
    ...
    for (let i = 0; i < firstDay; i++) {
        ...
        dpDatesGrid.appendChild(emptyCell);
    }
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        dpDatesGrid.appendChild(cell);
    }
    updateDpTopInput();
}

// أزرار التنقل بين الأشهر والسنوات
document.getElementById("dpPrevMonth").addEventListener("click", () => { dpCurrentMonth--; if (dpCurrentMonth < 0) { dpCurrentMonth = 11; dpCurrentYear--; } dpSelectedDate = null; renderCustomCalendar(); });
document.getElementById("dpNextMonth").addEventListener("click", () => { dpCurrentMonth++; if (dpCurrentMonth > 11) { dpCurrentMonth = 0; dpCurrentYear++; } dpSelectedDate = null; renderCustomCalendar(); });
document.getElementById("dpPrevYear").addEventListener("click", () => { dpCurrentYear--; dpSelectedDate = null; renderCustomCalendar(); });
document.getElementById("dpNextYear").addEventListener("click", () => { dpCurrentYear++; dpSelectedDate = null; renderCustomCalendar(); });

// القوائم المنسدلة للشهور والسنوات
dpMonthLabel.addEventListener("click", () => {
    dpDropdown.innerHTML = "";
    dpMonthNames.forEach((name, index) => {
        const item = document.createElement("div");
        item.className = "dropdown-item"; item.textContent = name;
        item.addEventListener("click", () => { dpCurrentMonth = index; dpSelectedDate = null; dpDropdown.style.display = "none"; renderCustomCalendar(); });
        dpDropdown.appendChild(item);
    });
    dpDropdown.style.display = "block";
});

dpYearLabel.addEventListener("click", () => {
    dpDropdown.innerHTML = "";
    for (let y = 2020; y <= 2050; y++) {
        const item = document.createElement("div");
        item.className = "dropdown-item"; item.textContent = y;
        item.addEventListener("click", () => { dpCurrentYear = y; dpSelectedDate = null; dpDropdown.style.display = "none"; renderCustomCalendar(); });
        dpDropdown.appendChild(item);
    }
    dpDropdown.style.display = "block";
});
...
document.querySelector(".picker").addEventListener("click", (e) => {
    if (e.target !== dpMonthLabel && e.target !== dpYearLabel && !dpDropdown.contains(e.target)) {
        dpDropdown.style.display = "none";
    }
});

// أزرار الأكشن السفلية (Cancel, Clear, Next)
document.getElementById("dpCancelBtn").addEventListener("click", closeCustomDatePicker);

document.getElementById("dpClearBtn").addEventListener("click", () => {
    if (activeDateInput) {
        const oldVal = activeDateInput.value;
        activeDateInput.value = '';
        const companyName = activeDateInput.closest('tr').cells[1].querySelector('input').value;
        addToActivityLog('التاريخ المتوقع', oldVal, 'فارغ', companyName);
        debouncedSaveAllData();
        reorderRows();
    }
...
document.getElementById("dpNextBtn").addEventListener("click", () => {
    if (activeDateInput && dpSelectedDate) {
        // تنسيق التاريخ المطلوب للبرنامج (YYYY-MM-DD)
        const formattedDate = `${dpCurrentYear}-${String(dpCurrentMonth + 1).padStart(2, '0')}-${String(dpSelectedDate).padStart(2, '0')}`;
        const oldVal = activeDateInput.value;
        
        if (oldVal !== formattedDate) {
            activeDateInput.value = formattedDate;
            const companyName = activeDateInput.closest('tr').cells[1].querySelector('input').value;
            addToActivityLog('التاريخ المتوقع', oldVal, formattedDate, companyName);
            debouncedSaveAllData();
            reorderRows(); // هذه الدالة ستقوم بتفعيل ألوان التنبيهات تلقائياً
        }
    }
    closeCustomDatePicker();
});